...
def get_named_platform() -> str:
    ...
    return f"✌️ lavHost {os.environ['LAVHOST']}" if "LAVHOST" in os.environ else "☠️ TrollHost"
...
def get_commit_url() -> str:
    ...
    try:
        hash_ = get_git_hash()
        return (
            f'<a href="https://github.com/trollub/troll/commit/{hash_}">#{hash_[:7]}</a>'
        )
    except Exception:
        return "Unknown"
...
def get_git_info() -> typing.Tuple[str, str]:
    ...
    return (
        hash_,
        f"https://github.com/trollub/troll/commit/{hash_}" if hash_ else "",
    )
